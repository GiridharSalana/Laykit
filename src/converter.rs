...
use crate::gdsii::{Boundary, GDSElement, GDSIIFile, GDSStructure, GDSTime, GPath, GText, StructRef};
use crate::oasis::{OASISCell, OASISElement, OASISFile, OPath, OText, Placement, Polygon, Rectangle};
...
pub fn gdsii_to_oasis(gds: &GDSIIFile) -> Result<OASISFile, Box<dyn std::error::Error>> {
    ...
    for (idx, structure) in gds.structures.iter().enumerate() {
        ...
        oasis.names.cell_names.insert(cell_ref, structure.name.clone());
    ...
    match element {
        GDSElement::Boundary(boundary) => {
            ...
            if is_rectangle(&boundary.xy) {
                ...
                // Convert to polygon
                let points: Vec<(i64, i64)> = boundary.xy.iter()
                ...
                let x = points[0].0;
                let y = points[0].1;
                let relative_points: Vec<(i64, i64)> = points.iter()
                    .map(|(px, py)| (*px - x, *py - y))
                    .collect();
        ...
        GDSElement::Path(path) => {
            let points: Vec<(i64, i64)> = path.xy.iter()
                .map(|(x, y)| (*x as i64, *y as i64))
                .collect();
        ...
        GDSElement::StructRef(sref) => {
            Some(OASISElement::Placement(Placement {
                ...
                mirror: sref.strans.as_ref().map(|st| st.reflection).unwrap_or(false),
                repetition: None,
                properties: Vec::new(),
            }))
        }
...
fn convert_oasis_element_to_gds(element: &OASISElement) -> Option<GDSElement> {
    match element {
        ...
        OASISElement::Polygon(poly) => {
            let xy: Vec<(i32, i32)> = poly.points.iter()
        ...
        OASISElement::Placement(placement) => {
            let strans = if placement.magnification.is_some() || placement.angle.is_some() || placement.mirror {
